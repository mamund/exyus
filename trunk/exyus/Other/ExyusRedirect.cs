...
namespace Exyus
{
    public class ExyusRedirect
    {
        ...
        public SortedList LoadRedirUriList()
        {
            ...
            string redirFile = util.GetConfigSectionItem(Constants.cfg_exyusSettings, Constants.cfg_redirectUrls);
        ...
        public string RedirLookUp(string uri)
        {
            ...
            else
            {
                ...
                for (int i = redirUriList.Count - 1; i > -1; i--)
                {
                    ...
                    {
                        // build up redirect
                        rtn = util.GetConfigSectionItem(Constants.cfg_exyusSettings, Constants.cfg_rootfolder) + rx.Replace(uri, redirUriList.GetByIndex(i).ToString());
                        rtn = rtn.Replace(".xcs", "");
        ...
        public SortedList LoadRewriteUriList()
        {
            ...
            HttpApplication app = HttpContext.Current.ApplicationInstance;
            int timeout = Convert.ToInt32(util.GetConfigSectionItem(Constants.cfg_exyusSecurity, Constants.cfg_authTimeout, "20"));
            string rewriteFile = util.GetConfigSectionItem(Constants.cfg_exyusSettings, Constants.cfg_rewriteUrls);
        ...
        public string RewriteLookUp(string uri)
        {
            ...
            else
            {
                ...
                for (int i = rewriteUriList.Count - 1; i > -1; i--)
                {
                    ...
                    {
                        // build up redirect
                        rtn = util.GetConfigSectionItem(Constants.cfg_exyusSettings,Constants.cfg_rootfolder)+rx.Replace(uri, rewriteUriList.GetByIndex(i).ToString());
